...
func TestSet(t *testing.T) {
	...
	dict.Set("key1", "value1") // should not add a new ont, just change the existing one
	if size := dict.Size(); size != 3 {
		t.Errorf("wrong count, expected 3 and got %d", size)
	}
...
func TestSize(t *testing.T) {
	...
	dict = populateDictionary(1000, 0)
	items = dict.Values()
	if len(items) != dict.Size() {
		t.Errorf("wrong count, exptected %d and got %d", dict.Size(), len(items))
	}
}
